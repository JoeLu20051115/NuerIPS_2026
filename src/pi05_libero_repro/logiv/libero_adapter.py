...
import threading
import time
from typing import Any, Callable, FrozenSet, Iterable, Iterator, Mapping, Protocol
...
def _observation_hash(observation: Mapping[str, Any]) -> str:
    ...
    for key in sorted(observation):
        ...
        if value.dtype.hasobject:
            raise ValueError(f"observation[{key!r}] cannot contain objects")
...
class TaskBinding:
    ...
    def from_manifest(cls, path: Path | str, task_id: int) -> "TaskBinding":
        ...
        if payload.get("frozen") is not True or task_ids != list(range(10)):
            raise ValueError("coverage manifest must be locked to exact task IDs 0..9")
        try:
            item = next(entry for entry in tasks if int(entry["task_id"]) == task_id)
        except StopIteration as error:
            raise ValueError(f"task {task_id} is outside the locked manifest") from error
