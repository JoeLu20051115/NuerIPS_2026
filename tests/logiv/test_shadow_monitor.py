...
def test_static_unplanned_support_is_only_an_anomaly_candidate() -> None:
    ...
    assert triggers[0].prior_failure_evidence == ()
...
def test_goal_regression_is_confirmed_only_after_goal_was_true() -> None:
    ...
    assert [item.evidence_kind for item in triggers[0].prior_failure_evidence] == [
        "GOAL_REGRESSION"
    ]
...
def test_attempted_effect_timeout_confirms_without_an_abnormal_surface() -> None:
    ...
    assert [
        item.evidence_kind for item in triggers[0].prior_failure_evidence
