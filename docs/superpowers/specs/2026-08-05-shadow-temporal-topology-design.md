...
the matching node `ACTIVE`.  Raw destination truth while holding marks it
`EFFECT_OBSERVED`.  A normalized destination effect marks it `COMPLETED`.
...
complete.  A completed effect that later regresses is not covered and may
make the certificate stale.
